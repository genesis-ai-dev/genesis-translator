...
export const updateLexicon = async (oldString: string,
    newString: string) => {
    if(vscode.workspace.workspaceFolders !== undefined) {
        const rootPath = vscode.workspace.workspaceFolders[0].uri;
        try {
            const lexiconUri = vscode.Uri.joinPath(rootPath, 'project/lexicon.csv');
            const lexiconContentUint8Array = await vscode.workspace.fs.readFile(lexiconUri);
            ...
            if (lines.length > lineIndex) {
                ...
                vscode.window.showInformationMessage('Line Content to update: ' + lineContent);
                const updatedLine = lineContent?.split(',').map((word: string) => {
                    if (word === oldString) {
                    return newString;
                    }
                    return word;
                ...
                vscode.window.showInformationMessage('Successfully processed lexicon.csv');
            } else {
